...
import sys
import time
from llm_api import encode_image_file
...
def query_llm(plan_content, user_prompt=None, file_content=None,image_content=None):
...
"""
    ...
    try:
        ...
        if image_content:
            ...
            messages[0]["content"] = [{"type": "text", "text": combined_prompt}]
...
def main():
    ...
    parser.add_argument('--files', type=str, nargs='+', help='Paths to files whose content should be included in the prompt', required=False)
    parser.add_argument('--image', type=str,help='Path to a image file to include in the prompt', required=False)
    ...
    image_content = None
    if args.image:
        encoded_image, mime_type = encode_image_file(args.image)
        image_content = f"data:{mime_type};base64,{encoded_image}"

    # Query LLM and output response
    response = query_llm(plan_content, args.prompt, file_content, image_content)
    if response:
